...
import { ControlCoordinator, isControlBusy, PendingAction } from "../src/session-restart.js";
...
test("index.ts dispatches /chat-new as a command with expandPromptTemplates", async () => {
	const source = await readFile(new URL("../index.ts", import.meta.url), "utf8");

	assert.ok(source.includes('registerCommand("chat-new"'), "index.ts must keep registerCommand(chat-new)");

	assert.ok(
		source.includes('pi.sendUserMessage("/chat-new"'),
		"remote new must dispatch /chat-new via pi.sendUserMessage",
	);
	assert.ok(
		source.includes("expandPromptTemplates: true"),
		"remote new must set expandPromptTemplates so pi dispatches the command (pi-chat#2)",
	);
